...
final class application
{
    ...
    {
        $query = "SELECT `application`.`id` as `id_application`,
                         `application`.`numerical_name`,
       
                         `application`.`id_expertise_purpose`,
                         `misc_expertise_purpose`.`name` AS `name_expertise_purpose`,
       
                         `application`.`additional_information`,
                         `application`.`object_name`,
       
                         `application`.`id_type_of_object`,
                         `misc_type_of_object`.`name` AS `name_type_of_object`,
       
                         `application`.`id_functional_purpose`,
                         `misc_functional_purpose`.`name` AS `name_functional_purpose`,
       
                         `application`.`id_functional_purpose_subsector`,
                         `misc_functional_purpose_subsector`.`name` AS `name_functional_purpose_subsector`,
       
                         `application`.`id_functional_purpose_group`,
                         `misc_functional_purpose_group`.`name` AS `name_functional_purpose_group`,
       
                         `application`.`number_planning_documentation_approval`,
                         `application`.`date_planning_documentation_approval`,
                         `application`.`number_GPZU`,
                         `application`.`date_GPZU`,
                        
                         `application`.`id_type_of_work`,
                         `misc_type_of_work`.`name` AS `name_type_of_work`,
       
                         `application`.`cadastral_number`,
       
                         `application`.`id_cultural_object_type`,
                         `misc_cultural_object_type`.`name` AS `name_cultural_object_type`,
       
                         `application`.`id_national_project`,
                         `misc_national_project`.`name` AS `name_national_project`,
       
                         `application`.`id_federal_project`,
                         `misc_federal_project`.`name` AS `name_federal_project`,
       
                         `application`.`date_finish_building`,
       
                         `application`.`id_curator`,
                         `misc_curator`.`name` AS `name_curator`
                  FROM (SELECT * FROM `application`
                        WHERE `application`.`id`=?) AS `application`
                  LEFT JOIN (`misc_expertise_purpose`)
                        ON (`application`.`id_expertise_purpose`=`misc_expertise_purpose`.`id`)
                  LEFT JOIN (`misc_type_of_object`)
                        ON (`application`.`id_type_of_object`=`misc_type_of_object`.`id`)
                  LEFT JOIN (`misc_functional_purpose`)
                        ON (`application`.`id_functional_purpose`=`misc_functional_purpose`.`id`)
                  LEFT JOIN (`misc_functional_purpose_subsector`)
                        ON (`application`.`id_functional_purpose_subsector`=`misc_functional_purpose_subsector`.`id`)
                  LEFT JOIN (`misc_functional_purpose_group`)
                        ON (`application`.`id_functional_purpose_group`=`misc_functional_purpose_group`.`id`)
                  LEFT JOIN (`misc_type_of_work`)
                        ON (`application`.`id_type_of_work`=`misc_type_of_work`.`id`)
                  LEFT JOIN (`misc_cultural_object_type`)
                        ON (`application`.`id_cultural_object_type`=`misc_cultural_object_type`.`id`)
                  LEFT JOIN (`misc_national_project`)
                        ON (`application`.`id_national_project`=`misc_national_project`.`id`)
                  LEFT JOIN (`misc_federal_project`)
                        ON (`application`.`id_federal_project`=`misc_federal_project`.`id`)
                  LEFT JOIN (`misc_curator`)
                        ON (`application`.`id_curator`=`misc_curator`.`id`)
                  ";
        $result = ParametrizedQuery::getFetchAssoc($query, [$id]);
