...
//       {result, error_message : текст ошибки}
//  6.3- Произошла внутренняя ошибка (по вине входных данных):
//       Загруженный файл не является открепленной подписью
//       {result, error_message : текст ошибки}
...
try {
    ...
    if ($signFileAssoc['file_size'] / 1024 > 20) {
        ...
        exit(json_encode([
            'result'        => 6.3,
            'error_message' => "Загруженный файл не является открепленной подписью"
        ]));
    }
    ...
    // Открепленная подпись не может содержать более одного подписанта
