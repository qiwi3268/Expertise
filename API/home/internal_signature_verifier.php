...
//       {result, error_message : текст ошибки}
//  4  - Произошла внутренняя ошибка (но по вине входных данных):
//       Получен недействительный тип криптографичесого сообщения (проверяется файл без встроенной подписи)
//       {result, error_message : текст ошибки}
//  5  - Произошла непредусмотренная ошибка при работе метода '\csp\Validator::validate'
//       {result, message : текст ошибки, code: код ошибки}
//  6  - Произошла ошибка при парсинге fs_name_sign
//       {result, message : текст ошибки, code: код ошибки}
...
// Получен недействительный тип криптографичесого сообщения.
/*
...
$P_mapping_level_1 = 1;
$P_mapping_level_2 = 1;
$P_fs_name_sign = '/var/www/applications_files/3151/53bbee8e68c3d85b542dc5895a344fc7066396793c2017ac3b7f4d15bded3fa350792d49837d9b43';
...
try{
    ...
    try{
        ...
        // Получение id файла
        list('application_id' => $applicationId, 'file_name' => $hash_sign) = ParseHelper::parseApplicationFilePath($P_fs_name_sign);
    ...
    $fileAssoc = $FileClassName::getAssocByIdApplicationAndHash($applicationId, $hash_sign);
