...
/**
 * Предназначен для преобразования первого символ строки в верхний регистр
 * с учетом многобайтовых кодировок
 *
 * @param string $str
 * @return string
 */
...
function getFIO(array $userInfo, bool $needShort = true): string
{
    ...
    if ($needShort) {
        ...
        // 1 группа:
        //    буква один и более раз
        // пробельный символ
        // 2 группа:
        //    буква
        // буква один и более раз
        // пробельный символ ноль или один раз
        // 3 группа: (ноль или один раз)
        //    буква
        // - использование кодировки utf-8
        $pattern = '/(\w+)\s(\w)\w+\s?(\w)?/u';

        $m = getHandlePregMatch($pattern, $full, false);

        return isset($m[3]) ? "{$m[1]} {$m[2]}.{$m[3]}." : "{$m[1]} {$m[2]}.";
    } else {
        return $full;
    }
}
