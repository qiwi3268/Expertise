...
class Navigation
{
    ...
    function __construct(array $userRoles)
    {
        ...
        $data = simplexml_load_file(NAVIGATION_SETTINGS);
    ...
    // <view />
    //    class_name класс, располагаемый по пути ROOT./Classes/Navigation/{class_name}.php
    //               класс должен быть наследником абстрактного класса NavigationTable
    //    view_name view, располагаемое по пути ROOT./views/home/navigation/{view_name}.php
    ...
    private function validateUserNavigation(): void
    {
        ...
        $abstract_class_path = ROOT . "/Classes/Navigation/{$abstract_class_name}.php";
        ...
        foreach ($this->userNavigation as $block) {
            ...
            if (isset($block['views'])) {
                ...
                foreach ($block['views'] as ['name' => $name, 'class_name' => $class_name, 'view_name' => $view_name, 'show_counter' => $show_counter]) {
                    ...
                    $class_path = ROOT . "/Classes/Navigation/{$class_name}.php";
                    ...
                    $view_path = ROOT . "/views/home/navigation/{$view_name}.php";
    ...
    private function validateNavigationSorting(): void
    {
        ...
        foreach ($uniqueViews as $view_name) {
            if (!isset(NAVIGATION_SORTING[$view_name])) {
                throw new Exception("В константном массиве NAVIGATION_SORTING отсутствует объявление view: '{$view_name}'");
            }
        }
    }
}