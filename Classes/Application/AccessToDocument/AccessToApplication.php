    ...
    public function checkAccess(): void
    {
        ...
        if (Session::isApp()) {
            ...
            if (!applicant_access_group::checkExistByIdApplicationAndIdUser($this->documentId, Session::getUserId())) {
                throw new SelfEx('Отсутствует доступ к документу "Заявление" для заявителя', 3001);
            }
            ...
            if (
                ...
                throw new SelfEx('Отсутствует доступ к документу "Заявление" для внештатного эксперта', 3002);
            }

            if (!assigned_expert_total_cc::checkExistByIdTotalCCAndIdExpert($totalCCId, Session::getUserId())) {
                throw new SelfEx('Отсутствует доступ к документу "Заявление" для внештатного эксперта', 3003);
            }
        }
